...
# Information from the Fastq file

  In this part, we use package dada2 and phyloseq to precess the fastq file of our microbiome data to get OTU tables. All the results has been run in our server and here are some representitive results about reads and how we truncated the sequence.
...
#  p1 <-  plotQualityProfile(fnFs[i]) + geom_vline(xintercept = 37,color = "blue") +  geom_vline(xintercept = 270,color = "blue")
#  p2 <- plotQualityProfile(fnRs[1]) + geom_vline(xintercept = 10,color = "blue") +  geom_vline(xintercept = 222,color = "blue")
#  grid.arrange(p1, p2)
#}
#dev.off()
...
ggplot(data = D_shannon) +  geom_point(aes(x = t, y = shannon, color = Treatment)) + geom_line(aes(x = t, y = shannon, color = Treatment, group = id)) + theme(axis.text.x = element_text(angle = 45)) +facet_wrap(~Treatment) + labs(title = "Plot of shannon alpha diversity") + theme_bw()

ggplot(data = D_simpson) +  geom_point(aes(x = t, y = simpson, color = Treatment)) + geom_line(aes(x = t, y = simpson, color = Treatment, group = id)) + theme(axis.text.x = element_text(angle = 45)) +facet_wrap(~Treatment) + labs(title = "Plot of simpson alpha diversity") + theme_bw()
...
df = cbind(out.pcoa3ra$vectors[,1:2],ps3@sam_data) %>% group_by(id) %>% arrange(t,.by_group = TRUE)
var_explained = out.pcoa3ra$values$Relative_eig[1:2]
...
df = cbind(out.pcoa3ra3$vectors[,1:2],ps3ra3@sam_data) %>% group_by(id) %>% arrange(t,.by_group = TRUE) 
...
df22 = df1 %>% filter((t == 0)|(t == 4)) %>%  group_by(id) %>% summarise(d04 = dist_temp(Axis.1,Axis.2))
df23 = df1 %>% filter((t == 2)|(t == 4)) %>%  group_by(id) %>% summarise(d24 = dist_temp(Axis.1,Axis.2))
id = unique(df$id[df$Treatment != "Control"])

df2 = data.frame(id = id, d_20 = df20$d_20[match(id,df20$id)], d02 = df21$d02[match(id,df21$id)],d04 = df22$d04[match(id,df22$id)], d24 = df23$d24[match(id,df23$id)])
...
confounder_p <- data.frame(var_name = "",test_type = "",p_20 = 1, p02 = 1, p04 = 1, p24 = 1)
...
for (i in 4:ncol(df3)){
  ...
  df_temp_04 <- na.omit(data.frame(y = y3,x = x))
  df_temp_24 <- na.omit(data.frame(y = y4,x = x))
  ...
  if (class(x) == "numeric"){
    ...
    p_04 = suppressMessages(try(cor.test(y = df_temp_04[,1], x = df_temp_04[,2], method = "spearman")$p.value,silent = T))
    ...
    p_04 = suppressMessages(try(wilcox.test(df_temp_04[,1] ~ as.factor(df_temp_04[,2]))$p.value,silent = T))
    ...
    p_04 = suppressMessages(try(anova(aov(df_temp_04[,1] ~ as.factor(df_temp_04[,2])))$`Pr(>F)`[1],silent = T))
    p_24 = suppressMessages(try(anova(aov(df_temp_24[,1] ~ as.factor(df_temp_24[,2])))$`Pr(>F)`[1],silent = T))
  }
  if (class(p_20) == "try-error" | class(p_02) == "try-error" | class(p_24) == "try-error"|class(p_04) == "try-error"){
    temp_testtype = "none"
    p_20 = 1
    p_02 = 1
    p_04 = 1
  ...
  temp <- data.frame(var_name = temp_varname,test_type = temp_testtype, p_20 = p_20, p02 = p_02,p04 = p_04, p24 = p_24)
...
pOTUs <- function(psgen, t1, t2){

  psgen1 <- subset_samples(psgen, (t == t1)|(t == t2))
  ...
  meta_data1$t <- factor(meta_data1$t,levels = c(t1,t2))
  meta_data1$Treatment <- as.factor(as.character(meta_data1$Treatment))
  
  mm <- model.matrix(id ~  Treatment, meta_data1)
  gen_clr <- aldex.clr(gen_table, mm, mc.samples = 128, denom = "all")
  
  glm.test <- aldex.glm(gen_clr)
  
  return(glm.test$`model.TreatmentDuodopa Pr(>|t|)`)
}
...
t1 = 0
t2 = c(-2,2,4)
p_20 <- pOTUs(psgen = psgen, t1 = t1, t2 = t2[1])
p02 <- pOTUs(psgen = psgen, t1 = t1, t2 = t2[2])
p04 <- pOTUs(psgen = psgen, t1 = t1, t2 = t2[3])
```

```{r}
p_Treatment <- data.frame(Genus = rep(as.data.frame(as.matrix(psgen@tax_table))[,6],2), p1 = rep(p_20,2), p2 = c(p02,p04), type = c(rep("p02",length(p02)),rep("p04",length(p04))))

```

```{r}
p = ggplot(p_Treatment) + geom_point(aes(x = -log(p1), y = -log(p2), color = type,  label = Genus)) + geom_abline(aes(intercept = 0, slope = 1)) + theme_classic()
ggplotly(p)
```
...
plotSpiec <- function(ps3, propthresh = 0.3, nlambda = 10, color = "Phylum"){
  ...
  ig2.mb <- adj2igraph(getRefit(se.ps3),  vertex.attr=list(name = taxa_names(ps31)))
